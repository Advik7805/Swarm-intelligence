...
watch(currentStage, (newStage) => {
  if (newStage === t('step2x.stagePersonas') || newStage === 'generating_profiles') {
    phase.value = 1
  } else if (newStage === t('step2x.stageConfig') || newStage === 'generating_config') {
    phase.value = 2
    // 进入配置生成阶段，开始轮询配置
    if (!configTimer) {
  ...
  } else if (newStage === t('step2x.stageScripts') || newStage === 'copying_scripts') {
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    console.warn(t('step2x.pollFailed') + ':', err)
  }
}
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    console.warn(t('step2x.profilesFailed') + ':', err)
  }
}
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    console.warn(t('step2x.configFailed') + ':', err)
  }
}
