...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      startError.value = res.error || t('step3x.startFailed')
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    console.warn(t('step3x.runStatusFailed') + ':', err)
  }
}
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn(t('step3x.detailStatusFailed') + ':', err)
  }
}
...
.timeline-axis {
  ...
  bottom: 0;
  width: 1px;
  background: var(--border); /* Cleaner line */
  transform: translateX(-50%);
}
...
.badge-action { background: var(--white); color: #666; border: 1px solid #E0E0E0; }
.badge-meta { background: var(--gray-light); color: #999; border: 1px dashed #DDD; }
