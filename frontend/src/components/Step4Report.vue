    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          if (interview.redditAnswer && interview.redditAnswer !== t('step4x.noReplyPlatform')) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== t('step4x.noReplyPlatform')) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === t('step4x.noReplyPlatform') || t === t('step4x.noReplyPlatform') || t === t('step4x.noReply')
    }
...
const activeStep = computed(() => {
  ...
  // 否则返回第一个步骤
  return steps[0] || { noLabel: '--', title: t('step4x.waitingStart'), status: 'todo', meta: '' }
})
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes(t('step4x.error'))) return 'error'
  if (log.includes('WARNING') || log.includes(t('step4x.warning'))) return 'warning'
...
.section-number {
  ...
  font-size: 16px;
  color: var(--wf-todo-text); /* 深灰色，不随状态变化 */
  font-weight: 500;
}
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid var(--gray-light); padding-bottom: 8px; }
...
.right-panel {
  ...
  --wf-divider: var(--gray-light);

  --wf-active-bg: var(--gray-light);
  --wf-active-border: var(--wf-active-text);
  --wf-active-dot: var(--wf-active-text);
  --wf-active-text: var(--wf-active-text);

  --wf-done-bg: var(--gray-light);
  --wf-done-border: var(--border);
  --wf-done-dot: var(--wf-done-dot);

  --wf-muted-dot: var(--wf-muted-dot);
  --wf-todo-text: var(--wf-todo-text);
}
...
:deep(.interview-display .expand-answer-btn) {
  ...
  background: transparent;
  border: none;
  border-bottom: 1px dotted var(--wf-muted-dot);
  border-radius: 0;
  font-size: 11px;
  font-weight: 500;
  color: var(--wf-todo-text);
...
:deep(.insight-display .rel-target) {
  padding: 4px 8px;
  background: var(--white);
  border: 1px solid var(--wf-muted-dot);
  border-radius: 4px;
  font-size: 11px;
  font-weight: 500;
  color: #374151;
